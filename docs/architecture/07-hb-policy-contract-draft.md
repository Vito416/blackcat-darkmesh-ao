...
- Non-off modes default to fail-open (`allow`) for rollout safety.
- Deny-capable behavior is active only after an admin-applied policy bundle sets
  `policyMode=soft|enforce` with `failOpen=false`.
