...
async function safeProcessSlot(baseUrl, pid) {
  const primary = `${baseUrl}/${pid}~process@1.0/slot/current?accept-bundle=true`
  const fallback = `${baseUrl}/${pid}/slot/current`
  const attempts = [primary, fallback]
  let last = null
  for (const url of attempts) {
    const probe = await safeFetch(url, { method: 'GET' }, 15000)
    probe.route = url === primary ? 'process' : 'legacy'
...
async function main() {
  ...
  for (const step of runReport.steps || []) {
    ...
    entry.probes.slotCurrentViaProcess = await safeProcessSlot(baseUrl, pid)
