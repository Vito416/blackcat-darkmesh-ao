...
- `bundle_export.lua` — bundle AO/Write exports into NDJSON.
- `catalog_feed.lua` — export products into NDJSON feed (PII-free).
- `sitemap.lua` — generate sitemap.xml from catalog state (requires `BASE_URL`).
...
# sitemap (BASE_URL required)
BASE_URL=https://example.com SITEMAP_PATH=/tmp/sitemap.xml \
  LUA_PATH="?.lua;?/init.lua;ao/?.lua;ao/?/init.lua" \
  lua5.4 scripts/export/sitemap.lua

# catalog feed
CATALOG_FEED_PATH=/tmp/catalog.ndjson \
  LUA_PATH="?.lua;?/init.lua;ao/?.lua;ao/?/init.lua" \
