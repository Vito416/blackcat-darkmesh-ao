...
if not ok_json then ok_json, cjson = pcall(require, "cjson") end
...
local function encode_fallback(obj)
  ...
  if t == "table" then
    ...
    for k, _ in pairs(obj) do
      idx = idx + 1
      if obj[idx] == nil then is_array = false break end
    end
    if is_array then
      local parts = {}
      for _, v in ipairs(obj) do table.insert(parts, encode_fallback(v)) end
